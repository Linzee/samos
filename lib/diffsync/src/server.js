...
Server.prototype.trackConnection = function(connection){
  ...
  connection.on(COMMANDS.leave, this.leaveConnection.bind(this, connection));
  connection.on('disconnect', this.disconnected.bind(this, connection));
};
...
Server.prototype.joinConnection = function(connection, room, initializeClient){
  this.getData(room, function(error, data){
    ...
    this.emit('connect', connection, room);
 ...
 * @param  {Function} initializeClient Callback that is being used for initialization of the client
 */
Server.prototype.leaveConnection = function(connection, room) {
  this.emit('disconnect', connection, room);
};
 ...
 * @param  {Function} initializeClient Callback that is being used for initialization of the client
 */
Server.prototype.disconnected = function(connection) {
  for(var room in this.data) {
    if(this.data[room].clientVersions.hasOwnProperty(connection.id)) {
      delete this.data[room].clientVersions[connection.id];
      this.leaveConnection(connection, room);
    }
  }
}
...
Server.prototype.receiveEdit = function(connection, editMessage, sendToClient){
  ...
  this.emit('edit', connection, editMessage);
...
Server.prototype.closeRoom = function(room) {
  ...
  this.adapter.storeData(room, undefined);
};
